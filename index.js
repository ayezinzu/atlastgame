...
client.on('message', async message => {
	...
	finalChannels.forEach((exactChannel, i) => {
		setTimeout(() => {
			...
			const cardmsg = client.channels
				.fetch(exactChannel)
				.then(channel => {
					return channel.send('*wooosh !* ***A card from Atlas Has spawned***', attachment1);
				})
				...
				.then(function (newmsg) {
					client.channels
						.fetch(exactChannel)
						.then(channel => {
							return channel.send('***The battle for the card begins in 15 seconds***');
						})
						.then(x => {
							setTimeout(() => {
								x.edit(
									'***The battle was lethal, thank goodness no one was hurt! \n Here are the results :***'
								);
							}, 15000);
			...
			setTimeout(() => {
				if (fighters.length > 2) {
					...
					client.channels.fetch(exactChannel).then(channel => {
						channel.send(
							`<@${randomWinner}> fought off ${map1}${noone} and won the ${cardname} \`\`${cardid}\`\` card worth +${cardscore}P`,
							attachment2
						);
					});
					...
					client.channels.fetch(exactChannel).then(channel => {
						channel.send(
							`Oh no! The \`\`${cardname}\`\` worth \`\`${cardscore} Pts\`\` got away!`,
							attachment2
						);
					});
				}
