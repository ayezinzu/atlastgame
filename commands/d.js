...
exports.run = (bot, message, args) => {
	if (talkedRecently.has(message.author.id)) {
		...
		if (cooldown) {
			...
			return message
				.reply(`you need to wait ${remaining} before using this command!`)
		...
		const cardmsg = message.channel
			.send('*wooosh !* ***A card from Atlas Has spawned***', attachment1)
			...
			.then(function (newmsg) {
				message.channel
					.send('***The battle for the card begins in 15 seconds***')
					.then(x => {
