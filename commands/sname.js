...
exports.run = async (client, message, args) => {
	...
	const arrayb = await Addcard.find({ cardname: { $regex: newargs, $options: 'i' } })
		.then(docs => {
			const transformedDocs = docs.map(
				(item, i) => `***${i + 1}.  \`\`${item.cardname}\`\` - ${item.series} \n*** `
			);
	...
	message.channel.send(
		'Reply with a number to view a card. You have 10 seconds to respond. Say ``stop`` if youd like to immediately perform another search. '
	);
	let array1 = [];
