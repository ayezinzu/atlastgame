...
exports.run = async (client, message, args) => {
	...
	newEmbed.addField(
		` \u200B`,
		`***Oh no ! <@${userid}> you are about to burn \`\`${cardname}\`\` to ashes ! you will get \`\`${cardscore}\`\` dust from it. Confirm this brutal act by reacting to \`\`👍\`\`. You have 10 seconds.*** `
	);
				...
				Report.findOne({ userid: userid }).then(function (datawegot) {
					datawegot.cardstats.forEach((item, i) => {
						if (item.cardid === cardid) {
							...
							cardscore = item.cardscore;
							imgurl = item.imgurl;
							const indexofcard = datawegot.cardstats
								.map(function (e) {
									return e.cardname;
								})
								.indexOf(cardname);
							...
							okEmbed.addField(
								` \u200B`,
								`***Riperoonie ;-; <@${userid}> just lit \`\`${cardname}\`\` on fire 🔥 and got \`\`${cardscore}\`\` dust from it.*** `
							);
