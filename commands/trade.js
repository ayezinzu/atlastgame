...
exports.run = async (client, message, args) => {
	...
	const traderone = await Canvas.loadImage(
		message.author.avatarURL({
			format: 'png'
		})
	);
	const tradertwo = await Canvas.loadImage(
	...
	newEmbed.addField(
		` \u200B`,
		`***Heyo <@${userid1}> your friend <@${userid}> wants to give you \`\`${cardname}\`\` worth \`\`${
			cardscore + upgrade
		}\`\` pts  in return of \`\`${cardname1}\`\` worth \`\`${
			cardscore1 + upgrade1
		}\`\`pts. To accept this trade react to \`\`👍\`\`. You have 10 seconds.*** `
	);
	...
	tradecancelEmbed.addField(
		`\u200B`,
		` <@${userid}> did not recieve \n**\`\`${cardid1}\`\` - ${cardname1} - ${
			cardscore1 + upgrade1
		} pts - ${series11}**`
	);
	tradecancelEmbed.addField(
		`\u200B`,
		` <@${userid1}> did not recieve \n**\`\`${cardid}\`\` - ${cardname} - ${cardscore + upgrade} pts - ${series}**`
	);
	...
	const filter = (reaction, user) => {
		return (
			...
			[message.author.id, message.mentions.members.first().id].includes(user.id)
		);
	};
	...
	burnmsg
		...
		.then(async collected => {
			...
			if (reaction.emoji.name === '👍') {
				...
				const indexOfUserOne = user_one.cardstats
					.map(function (emy) {
						return emy.cardid;
					})
					.indexOf(cardid);
				...
				user_one
					.save()
					.then(resultadd => console.log('saved data'))
				...
				const indexOfUserTwo = user_two.cardstats
					.map(function (my) {
						return my.cardid;
					})
					.indexOf(cardid1);
				...
				user_two
					.save()
					.then(resultf => console.log('saved data'))
					.catch(err => console.log(err));
