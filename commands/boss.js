...
for (itemone of newInput.cardsarray){
   ...
   let winCalculator = Math.random() * 100
    ...
    if (
      winCalculator < winchance
    ) {
      const rewardsEmbed = new Discord.MessageEmbed().setTitle('Amazing fight warriors, you did it!')
      console.log('we in')
  ...
  async function giveDust(addDust, rewardHim){
    ...
    winmsg.push(
      `<@${
        rewardHim.userid
      }> won \`\`${addDust} dust\`\` from the boss fight `
    );
  ...
  async function giveReward(typeOfRewardCard, rewardHim){
    ...
    winmsg.push(
      `<@${rewardHim.userid}> won \`\`${card.cardname} - (${
        card.cardtype
      }) - ${card.element}\`\` from the boss fight `
    );
    rewardsEmbed.addField(` ‎‏‏‎ ‎‏‏‎ ‎‏‏‎ ‎‏‏‎`, winmsg)
  ...
  for (itemone of newInput.cardsarray){
    ...
    if(bossdifficulty === 1){
      ...
      theReward = rewardsArray[Math.floor(Math.random() * rewardsArray.length)]
    ...
    if(bossdifficulty === 2){
      ...
      theReward = rewardsArray[Math.floor(Math.random() * rewardsArray.length)]
      console.log(theReward, `the reward`)
      if(theReward === 2 || theReward === 3 || theReward === 4 || theReward === 1){
    ...
    if(bossdifficulty === 5){
      ...
      theReward = rewardsArray[Math.floor(Math.random() * rewardsArray.length)]
      console.log(theReward, `the reward`)
      if(theReward === 4 || theReward === 5 || theReward === 6 || theReward === 7){
        await giveDust(theReward, rewardThisGuy)
      }
      else if(theReward === 1111){
        times = 1
...
rewardsEmbed.setThumbnail(`https://cdn.discordapp.com/attachments/755043428550574120/755293921847804007/cup.png`)
...
console.log(`ok everything`)
      ...
      const lossEmbed = new Discord.MessageEmbed().setTitle('Rest in peace our beloved warriors.')
      lossEmbed.setImage(`https://cdn.discordapp.com/attachments/718036708435689494/755285630308581406/funeral.png`)
