	...
	var interval = setInterval(function () {
		...
		const cardmsg = message.channel
			...
			.then(function (newmsg) {
				message.channel
					.send('***The battle for the card beginds in 10 seconds***')
					.then(x => {
						setTimeout(() => {
							x.edit(
								'***The battle was lethal, thank goodness no one was hurt! Here are the results :***'
							);
						}, 5000);
		...
		setTimeout(() => {
			if (fighters.length > 2) {
				...
				message.channel.send(
					`<@${randomWinner}> fought off ${map1} and won the xyz card worth xyz points`,
					attachment2
				);
			}
		}, 10000);
